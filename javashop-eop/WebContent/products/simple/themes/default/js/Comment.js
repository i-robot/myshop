...
var CommentList={
		...
		loadDiscussList:function(page){
		 
			var self  = this;
			$("#discuss_list_wrapper").load("goods_comments.html?widgetid=comment&action=discuss_list&goods_id="+goodsid+"&ajax=yes&page="+page,function(){
				self.bindDiscussPageEvent();			
				
			});
		},
		loadAskList:function(page){
		 
			var self  = this;
			$("#ask_list_wrapper").load("goods_comments.html?widgetid=comment&action=ask_list&goods_id="+goodsid+"&ajax=yes&page="+page,function(){
				self.bindAskPageEvent();	
...
var CommentForm={
	init:function(){		
		var self  = this;		
		self.loadDiscussForm();
		self.loadAskForm();
	},
	
	formSubmit:function(btn,form){
		var options = { 
				url : "widget.do?type=comment&action=add&ajax=yes",
				type : "POST",
				dataType : "json",
	...
	//加载评论表单
	loadDiscussForm:function(){
		$("#discuss_form_wrapper").load("goods_comments.html?ajax=yes&widgetid=comment&action=discuss_form&goodsid="+goodsid,function(){		 
			Grade.init();
		});
	},
	...
	loadAskForm:function(){ 
		$("#ask_form_wrapper").load("goods_comments.html?ajax=yes&widgetid=comment&action=ask_form&goodsid="+goodsid);
	}
}
